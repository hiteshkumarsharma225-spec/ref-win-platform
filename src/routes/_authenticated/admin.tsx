...
function AdminPage() {
  ...
  const approveWithdrawal=useMutation({
    mutationFn:async(id:string)=>{const {error}=await supabase.rpc("admin_approve_virtual_credit_withdrawal",{p_id:id});if(error)throw error;},
    onSuccess:()=>{qc.invalidateQueries({queryKey:["admin-credit-withdrawals"]});toast.success("Withdrawal processed");},onError:(e:Error)=>toast.error(e.message)
  });
  const completeWithdrawal=useMutation({
    mutationFn:async(id:string)=>{const {error}=await supabase.rpc("admin_complete_virtual_credit_withdrawal",{p_id:id});if(error)throw error;},
    onSuccess:()=>{qc.invalidateQueries({queryKey:["admin-credit-withdrawals"]});toast.success("Withdrawal completed");},onError:(e:Error)=>toast.error(e.message)
  });
...
function Section({title,subtitle,children}:{title:string;subtitle:string;children:React.ReactNode}){return <section className="mt-4 space-y-3"><div><h2 className="font-display text-lg font-bold">{title}</h2><p className="text-xs text-muted-foreground">{subtitle}</p></div>{children}</section>}
